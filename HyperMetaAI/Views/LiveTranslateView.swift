...
struct LiveTranslateView: View {
    ...
    private var videoBackground: some View {
        ...
        .onChange(of: streamViewModel.currentVideoFrame) { _, frame in
            if let frame, streamViewModel.cameraCaptureState.isStreaming {
    ...
    private func startVideoStream() {
        Task {
            let started = await streamViewModel.acquireStream(for: .liveTranslate)
            guard started else {
                await streamViewModel.releaseStream(for: .liveTranslate)
                closeAndRelease()
                return
            }
        }
...
#if DEBUG
@MainActor
private struct LiveTranslatePreview: View {
    @StateObject private var dependencies = PreviewDependencies()

    var body: some View {
        LiveTranslateView(streamViewModel: dependencies.streamViewModel)
    }
}

#Preview("Live Translate") {
    LiveTranslatePreview()
}
#endif
