...
@MainActor
private struct MockDeviceCardPreview: View {
  @StateObject private var dependencies = PreviewDependencies()

  var body: some View {
    if let device = MockDeviceKit.shared.pairedDevices.first {
      MockDeviceCardView(
        viewModel: MockDeviceCardView.ViewModel(
          device: device,
          hasCameraFeed: true,
...
#Preview("Mock Device Card") {
  MockDeviceCardPreview()
}
