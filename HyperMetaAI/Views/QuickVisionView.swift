...
struct QuickVisionView: View {
    ...
    private var buttonDisabled: Bool {
        quickVisionManager.isProcessing || !streamViewModel.cameraCaptureState.isStreaming
    }
    ...
    var body: some View {
        ...
        .task {
            ...
            guard streamViewModel.hasActiveDevice, !isClosing else {
        ...
        .onDisappear {
            guard !AppIdentity.isRunningPreview, !isClosing else { return }

            isClosing = true
            quickVisionManager.cancelCurrentRequest()
            Task { @MainActor in
                await quickVisionManager.stopStream()
            }
        }
    }
            ...
            if let photo = quickVisionManager.lastImage {
                ...
                RoundedRectangle(cornerRadius: AppCornerRadius.lg)
                    ...
                    .overlay {
                        ...
                        } else if streamViewModel.cameraCaptureState.isBusy || quickVisionManager.isProcessing {
                            VStack(spacing: AppSpacing.md) {
                                ProgressView()
                                    .scaleEffect(1.5)
                                    .tint(.white)
...
#if DEBUG
@MainActor
private struct QuickVisionPreview: View {
