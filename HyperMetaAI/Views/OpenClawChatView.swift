...
struct OpenClawChatView: View {
            ...
            VStack(spacing: 0) {
                ...
                VStack(spacing: 12) {
                    ...
                    HStack(spacing: 16) {
                        ...
                        .disabled(
                            isSending ||
                            openClawService.connectionState != .connected ||
    ...
    private func snapAndSend() async {
        ...
        let streamReady = await streamViewModel.acquireStream(for: .openClawChat)
        guard streamReady else {
            await streamViewModel.releaseStream(for: .openClawChat)
...
private struct OpenClawChatPreview: View {
    ...
    var body: some View {
        OpenClawChatView(streamViewModel: dependencies.streamViewModel)
    }
}

#Preview("OpenClaw Chat") {
    OpenClawChatPreview()
}
#endif
