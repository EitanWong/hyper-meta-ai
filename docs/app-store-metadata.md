...
• 眼镜触发语音会话：单击开始、再次单击结束、长按结束；App 不显示暂停状态，会话状态实时同步到镜片菜单。
...
注意：App 不使用 iPhone 相机，画面来自智能眼镜（需 Meta View App 完成配网）；无眼镜也可使用 iPhone 麦克风、扬声器和聊天功能。
```
